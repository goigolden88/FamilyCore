 ...
 * (docs/02-Архитектура.md, «Правила»; Р-75 «Дневников»).
 *
 * Почему Git Data API, а не простой Contents API (Р-32 «Дневников»): Contents пишет по
...
export function createClient({ repo, token, fetch = globalThis.fetch }: ClientOptions) {
  ...
  async function explain(response: Response): Promise<GitHubError> {
    ...
    if (status === 401) {
      return new GitHubError(
        ...
        // бессрочно (Р-67 «Дневников»). Отказ бывает и у отозванного, и у вставленного
        // не целиком.
    ...
    if (status === 409 || status === 422) {
      ...
      return new GitHubError(
        ...
        // не говорит — называем то, что случилось (Р-62 «Дневников»).
        empty
  ...
  return {
    ...
    /**
     * Один коммит на все файлы (Р-32 «Дневников»).
     *
 ...
 *
 * Нужен, чтобы не отправлять файлы, которые не изменились (Р-33 «Дневников»): считаем
